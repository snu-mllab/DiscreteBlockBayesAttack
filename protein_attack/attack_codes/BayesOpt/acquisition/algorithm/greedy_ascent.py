...
from attack_codes.BayesOpt.dpp.dpp import dpp_sample
import copy 
...
def greedy_ascent_with_indices(center_indices, opt_indices, stage, hb, surrogate_model, kernel_name, batch_size, reference=None, filter=True, return_ei=False, acq_with_opt_indices=True, reg_coef=0.0):
    ...
    if reference == None:
        ...
        reference -= reg_coef *  torch.count_nonzero(hb.eval_X_num[best_ind])

    if len(remained_indices)==0:
        "something wrong, do larger space in greedy ascent with indices"
        if return_ei:
            center_ei = surrogate_model.acquisition(centerX, bias=reference, reg_coef=reg_coef)
            return center_indices.view(1,-1), torch.Tensor([center_ei])
        else:
            return center_indices.view(1,-1)

    ei = surrogate_model.acquisition(testX, bias=reference, reg_coef=reg_coef)
...
def acquisition_maximization_with_indices(cur_seqs, opt_indices, batch_size, stage, hb, surrogate_model, kernel_name, reference=None, patience=5, dpp_type='no', acq_with_opt_indices=True, reg_coef=0.0):
    global_candidates_, global_eis_ = [], []

    for cur_seq in cur_seqs:
        cur_indices = hb.numbering_seq(cur_seq).view(1,-1)
        if acq_with_opt_indices:
            cur_ei = surrogate_model.acquisition(cur_indices[:,opt_indices], bias=reference, reg_coef=reg_coef)
        else:
            cur_ei = surrogate_model.acquisition(cur_indices, bias=reference, reg_coef=reg_coef)
        ...
        new_candidates_ = []
        new_eis_ = []
        new_candidates, new_eis = greedy_ascent_with_indices(cur_indices, opt_indices, stage, hb, surrogate_model, kernel_name=kernel_name, batch_size=num_next, reference=reference, filter=filtering, return_ei=True, acq_with_opt_indices=acq_with_opt_indices, reg_coef=reg_coef)
        ...
        #topk_values, topk_indices = torch.topk(eis, min(len(eis),num_next))
        #candidates = torch.cat([candidates[ind].view(1,-1) for ind in topk_indices], dim=0)
        #eis = eis[topk_indices]
